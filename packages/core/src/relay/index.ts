...
export const sponsoredCall = async (request: SponsoredCallRequest): Promise<Hash> => {
...
export const syncPaymentCall = async (request: SponsoredCallRequest): Promise<Hash> => {
  if (request.authorizationList && request.authorizationList.length > 0)
    delete request.authorizationList[0].v;
